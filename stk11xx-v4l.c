...
static int stk11xx_querycap(struct file *fp, void *fh, struct v4l2_capability *cap) {
    struct usb_stk11xx *dev;
...
static int stk11xx_queryctrl(struct file *fp, void *fh, struct v4l2_queryctrl *c) {
    int i;
    int nbr;
    struct usb_stk11xx *dev;
...
static int stk11xx_g_ctrl(struct file *fp, void *fh, struct v4l2_control *c) {
    struct usb_stk11xx *dev;
...
static int stk11xx_s_ctrl(struct file *fp, void *fh, struct v4l2_control *c) {
    struct usb_stk11xx *dev;
...
static int stk11xx_enum_input(struct file *fp, void *fh, struct v4l2_input *i) {
    struct usb_stk11xx *dev;
...
static int stk11xx_g_input(struct file *fp, void *fh, unsigned int *i) {
    struct usb_stk11xx *dev;
    STK_DEBUG("GET INPUT\n");
...
static int stk11xx_s_input(struct file *fp, void *fh, unsigned int i) {
    struct usb_stk11xx *dev;
...
static int stk11xx_enum_fmt_vid_cap(struct file *fp, void *fh, struct v4l2_fmtdesc *fmt) {
    int index;
...
static int stk11xx_try_fmt_vid_cap(struct file *fp, void *fh, struct v4l2_format *fmt) {
    struct usb_stk11xx *dev;
    dev = video_get_drvdata(video_devdata(fp));
    ...
    switch (dev->webcam_type) {
        case STK11XX_SXGA:
            if (fmt->fmt.pix.width > stk11xx_image_sizes[STK11XX_1280x1024].x)
                fmt->fmt.pix.width = stk11xx_image_sizes[STK11XX_1280x1024].x;
            else if (fmt->fmt.pix.width < stk11xx_image_sizes[0].x)
                fmt->fmt.pix.width = stk11xx_image_sizes[0].x;

            if (fmt->fmt.pix.height > stk11xx_image_sizes[STK11XX_1280x1024].y)
                fmt->fmt.pix.height = stk11xx_image_sizes[STK11XX_1280x1024].y;
            else if (fmt->fmt.pix.height < stk11xx_image_sizes[0].y)
                fmt->fmt.pix.height = stk11xx_image_sizes[0].y;
            break;

        case STK11XX_PAL:
            if (fmt->fmt.pix.width > stk11xx_image_sizes[STK11XX_720x576].x)
                fmt->fmt.pix.width = stk11xx_image_sizes[STK11XX_720x576].x;
            else if (fmt->fmt.pix.width < stk11xx_image_sizes[0].x)
                fmt->fmt.pix.width = stk11xx_image_sizes[0].x;

            if (fmt->fmt.pix.height > stk11xx_image_sizes[STK11XX_720x576].y)
                fmt->fmt.pix.height = stk11xx_image_sizes[STK11XX_720x576].y;
            else if (fmt->fmt.pix.height < stk11xx_image_sizes[0].y)
                fmt->fmt.pix.height = stk11xx_image_sizes[0].y;
            break;

        case STK11XX_VGA:
            if (fmt->fmt.pix.width > stk11xx_image_sizes[STK11XX_640x480].x)
                fmt->fmt.pix.width = stk11xx_image_sizes[STK11XX_640x480].x;
            else if (fmt->fmt.pix.width < stk11xx_image_sizes[0].x)
                fmt->fmt.pix.width = stk11xx_image_sizes[0].x;

            if (fmt->fmt.pix.height > stk11xx_image_sizes[STK11XX_640x480].y)
                fmt->fmt.pix.height = stk11xx_image_sizes[STK11XX_640x480].y;
            else if (fmt->fmt.pix.height < stk11xx_image_sizes[0].y)
                fmt->fmt.pix.height = stk11xx_image_sizes[0].y;
            break;
    }
    ...
    switch (fmt->fmt.pix.pixelformat) {
        ...
        case V4L2_PIX_FMT_BGR24:
            ...
            fmt->fmt.pix.bytesperline = 3 * fmt->fmt.pix.width;
            break;
        ...
        case V4L2_PIX_FMT_BGR32:
            ...
            fmt->fmt.pix.bytesperline = 4 * fmt->fmt.pix.width;
            break;
...
static int stk11xx_g_fmt_vid_cap(struct file *fp, void *fh, struct v4l2_format *fmt) {
    ...
    memcpy(&(fmt->fmt.pix), &pix_format, sizeof(pix_format));

    return 0;
}

static int stk11xx_s_fmt_vid_cap(struct file *fp, void *fh, struct v4l2_format *fmt) {
    struct usb_stk11xx *dev;
    ...
    if (v4l_stk11xx_select_video_mode(dev, fmt->fmt.pix.width, fmt->fmt.pix.height)) {
...
static int stk11xx_enum_framesizes(struct file *fp, void *fh, struct v4l2_frmsizeenum *frm) {
    STK_DEBUG("IOCTL unknown !\n");
    return -ENOIOCTLCMD;
}

static int stk11xx_reqbufs(struct file *fp, void *fh, struct v4l2_requestbuffers *rb) {
    int nbuffers;
    struct usb_stk11xx *dev;
...
static int stk11xx_querybuf(struct file *fp, void *fh, struct v4l2_buffer *buf) {
    int index;
    struct usb_stk11xx *dev;
...
static int stk11xx_qbuf(struct file *fp, void *fh, struct v4l2_buffer *buf) {
    struct usb_stk11xx *dev;
...
static int stk11xx_dqbuf(struct file *fp, void *fh, struct v4l2_buffer *buf) {
    struct usb_stk11xx *dev;
    int ret;
...
static int stk11xx_streamon(struct file *fp, void *fh, enum v4l2_buf_type arg) {
    struct usb_stk11xx *dev;
...
static int stk11xx_streamoff(struct file *fp, void *fh, enum v4l2_buf_type arg) {
    struct usb_stk11xx *dev;
...
static long v4l_stk11xx_do_ioctl(struct file *fp,
    ...
    switch (cmd) {
        ...
        case VIDIOC_QUERYCAP: {
            return stk11xx_querycap(fp, NULL, arg);
        }
        case VIDIOC_ENUMINPUT: {
            return stk11xx_enum_input(fp, NULL, arg);
        }
        case VIDIOC_G_INPUT: {
            return stk11xx_g_input(fp, NULL, arg);
        }
        case VIDIOC_S_INPUT: {
            return stk11xx_s_input(fp, NULL, *((int *) arg));
        }
        case VIDIOC_QUERYCTRL: {
            return stk11xx_queryctrl(fp, NULL, arg);
        }
        case VIDIOC_G_CTRL: {
            return stk11xx_g_ctrl(fp, NULL, arg);
        }
        case VIDIOC_S_CTRL: {
            return stk11xx_s_ctrl(fp, NULL, arg);
        }
        case VIDIOC_ENUM_FMT: {
            return stk11xx_enum_fmt_vid_cap(fp, NULL, arg);
        }
        case VIDIOC_G_FMT: {
            return stk11xx_g_fmt_vid_cap(fp, NULL, arg);
        }
        case VIDIOC_TRY_FMT: {
            return stk11xx_try_fmt_vid_cap(fp, NULL, arg);
        }
        case VIDIOC_S_FMT: {
            return stk11xx_s_fmt_vid_cap(fp, NULL, arg);
        }
        ...
        case VIDIOC_REQBUFS: {
            return stk11xx_reqbufs(fp, NULL, arg);
        }
        case VIDIOC_QUERYBUF: {
            return stk11xx_querybuf(fp, NULL, arg);
        }
        case VIDIOC_QBUF: {
            return stk11xx_qbuf(fp, NULL, arg);
        }
        case VIDIOC_DQBUF: {
            return stk11xx_dqbuf(fp, NULL, arg);
        }
        case VIDIOC_STREAMON: {
            return stk11xx_streamon(fp, NULL, *((enum v4l2_buf_type *) arg));
        }
        case VIDIOC_STREAMOFF: {
            return stk11xx_streamoff(fp, NULL, *((enum v4l2_buf_type *) arg));
        }
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 18, 0)
static const struct v4l2_ioctl_ops stk11xx_ioctl_ops = {
        ...
        .vidioc_enum_fmt_vid_cap  = stk11xx_enum_fmt_vid_cap,
        .vidioc_try_fmt_vid_cap   = stk11xx_try_fmt_vid_cap,
        .vidioc_g_fmt_vid_cap     = stk11xx_g_fmt_vid_cap,
        .vidioc_s_fmt_vid_cap     = stk11xx_s_fmt_vid_cap,

        .vidioc_enum_framesizes   = stk11xx_enum_framesizes,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 18, 0)
    dev->vdev->ioctl_ops = &stk11xx_ioctl_ops;
#endif
